...
export default function FichasLista() {
  ...
  const insertFichaImpressa = async (
    ...
    if (isKit) {
      ...
      console.info(`[Kit] Venda registrada: ${item.ficha.nome_produto}, valor total: ${kitValorTotal}`);
    } else {
      ...
      <ConfirmDialog
        ...
        onConfirm={async () => {
          ...
          try {
            ...
            // Insert into fichas_impressas (kit stock handled inside insertFichaImpressa)
            const sbClient = await getSupabaseClient();
            try {
              const codigoVenda = generateCodigoVenda();
              for (const ci of cart) {
