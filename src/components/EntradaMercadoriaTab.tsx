...
import { Plus, Trash2, Save, Search, Package, Pencil, Trash } from 'lucide-react';
...
export default function EntradaMercadoriaTab() {
  ...
  const [editingEntrada, setEditingEntrada] = useState<EntradaHeader | null>(null);
  ...
  // Delete confirm
  const [deleteTarget, setDeleteTarget] = useState<EntradaHeader | null>(null);
  ...
  const openEditModal = async (entrada: EntradaHeader) => {
    setEditingEntrada(entrada);
    setNumeroNota(entrada.numero_nota);
    setDataCompra(entrada.data_compra);
    ...
    if (data) {
      setItens((data as any[]).map(d => ({
        produto_id: d.produto_id,
        produto_nome: d.produto_nome,
        quantidade: d.quantidade,
        valor_comprado: Number(d.valor_comprado),
        margem_lucro: Number(d.margem_lucro),
      })));
    }
  };
    ...
    try {
      ...
      if (editingEntrada) {
        ...
        const { error: errH } = await supabase
          ...
          .update({ numero_nota: numeroNota.trim(), data_compra: dataCompra, observacao: observacao.trim() || null } as any)
  ...
  const handleDelete = async () => {
    ...
    try {
      const supabase = await getSupabaseClient();
      await supabase.from('entradas_mercadoria_itens' as any).delete().eq('entrada_id', deleteTarget.id);
      const { error } = await supabase.from('entradas_mercadoria' as any).delete().eq('id', deleteTarget.id);
      if (error) throw error;
      toast({ title: 'Entrada excluída com sucesso!' });
      setDeleteTarget(null);
      fetchData();
    } catch (err: any) {
      toast({ title: 'Erro ao excluir', description: err?.message || 'Erro desconhecido', variant: 'destructive' });
    } finally {
      setDeleting(false);
    }
  };
    ...
    <TooltipProvider>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <Button onClick={openNewModal}><Plus className="h-4 w-4 mr-2" /> Adicionar nota</Button>
        </div>
        ...
        {entradas.length === 0 ? (
          ...
          <div className="rounded-md border overflow-x-auto">
            <Table>
              ...
              <TableBody>
                {entradas.map(e => {
                  const canEdit = isToday(e.created_at);
                  return (
                        ...
                        <div className="flex items-center justify-center gap-1">
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <span>
                                <Button size="icon" variant="ghost" className="h-8 w-8" disabled={!canEdit} onClick={() => openEditModal(e)}>
                                  <Pencil className="h-4 w-4" />
                                </Button>
                              </span>
                            </TooltipTrigger>
                            {!canEdit && <TooltipContent>Só é possível editar notas lançadas hoje.</TooltipContent>}
                          </Tooltip>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <span>
                                <Button size="icon" variant="ghost" className="h-8 w-8 text-destructive" disabled={!canEdit} onClick={() => setDeleteTarget(e)}>
                                  <Trash className="h-4 w-4" />
                                </Button>
                              </span>
                            </TooltipTrigger>
                            {!canEdit && <TooltipContent>Só é possível excluir notas lançadas hoje.</TooltipContent>}
                          </Tooltip>
                        </div>
                      </TableCell>
                    </TableRow>
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div className="space-y-1">
                  <Label>Usuário</Label>
                  <Input value={editingEntrada?.usuario_nome || access?.nome || ''} disabled />
                </div>
              </div>
              ...
              <Button onClick={handleSave} disabled={saving} className="w-full sm:w-auto">
                ...
                {saving ? 'Salvando...' : editingEntrada ? 'Atualizar entrada' : 'Salvar entrada'}
              </Button>
            </div>
          </DialogContent>
        </Dialog>
        ...
        <ConfirmDialog
          ...
          description={`Deseja realmente excluir a nota "${deleteTarget?.numero_nota}"? Esta ação não pode ser desfeita.`}
