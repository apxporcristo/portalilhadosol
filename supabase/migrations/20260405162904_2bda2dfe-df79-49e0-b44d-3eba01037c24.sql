...
SELECT
  ...
  p.categoria_id,
  c.nome_categoria,
  p.nome_produto AS nome_item,
  p.obs AS observacao,
  'produto'::text AS tipo_item,
  NULL::uuid AS produto_principal_id,
  p.valor,
  p.ativo,
...
SELECT
  ...
  k.categoria_id,
  c.nome_categoria,
  k.nome_kit AS nome_item,
  k.observacao,
  'kit'::text AS tipo_item,
  k.produto_principal_id,
  0::numeric AS valor,
  k.ativo,
  c.exigir_dados_cliente,
  c.exigir_dados_atendente,
  'unitario'::text AS forma_venda,
  0::numeric AS valor_por_kg,
  NULL::uuid AS printer_id,
  k.created_at,
  k.updated_at
FROM fichas_kits k
